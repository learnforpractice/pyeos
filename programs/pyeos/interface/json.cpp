...
template<typename T, json::parse_type parser_type> variant variant_from_stream(
      T& in);
...
template<typename T, json::parse_type parser_type> variant_object objectFromStream(
      T& in);
template<typename T, json::parse_type parser_type> variants arrayFromStream(
      T& in);
template<typename T, json::parse_type parser_type> variant number_from_stream(
      T& in);
...
variant_object objectFromStream(T& in) {
   ...
   try {
      ...
      if (c != '{')
         FC_THROW_EXCEPTION(parse_error_exception,
               "Expected '{', but read '${char}'", ("char",string(&c, &c + 1)));
      in.get();
      skip_white_space(in);
      while (in.peek() != '}') {
         if (in.peek() == ',') {
...
variants arrayFromStream(T& in) {
   ...
   try {
      ...
      if (in.peek() != ']')
         FC_THROW_EXCEPTION(parse_error_exception,
               "Expected ']' after parsing ${variant}", ("variant", ar));
...
variant number_from_stream(T& in) {
   ...
   if (dot)
      return
            parser_type == json::legacy_parser_with_string_doubles ?
                  variant(str) : variant(to_double(str));
   if (neg)
      return to_int64(str);
...
PyObject* json::to_string(const variant& v,
