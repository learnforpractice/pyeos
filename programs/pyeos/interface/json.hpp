...
public:
   ...
   enum output_formatting {
      stringify_large_ints_and_doubles = 0, legacy_generator = 1
   };
   ...
   static ostream& to_stream(ostream& out, const variant_object& v,
         output_formatting format = stringify_large_ints_and_doubles);
   static PyDict& to_stream(PyDict& dict, const variants& v,
         output_formatting format = stringify_large_ints_and_doubles);

   static variant from_string(const string& utf8_str, parse_type ptype =
         legacy_parser);
   static variants variants_from_string(const string& utf8_str,
         parse_type ptype = legacy_parser);
   static PyObject* to_string(const variant& v, output_formatting format =
         stringify_large_ints_and_doubles);

   static bool is_valid(const std::string& json_str, parse_type ptype =
         legacy_parser);
   ...
   template<typename T>
   static PyObject* to_string(const T& v, output_formatting format =
         stringify_large_ints_and_doubles) {
