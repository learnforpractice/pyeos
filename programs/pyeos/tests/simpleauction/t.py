...
def bid(price):
    ...
    with producer:
        ...
        r = eosapi.push_action('auction1','bid', msg, {'auction1':'active'})
        assert r
    '''
    with producer:
        msg = {"from":"hello", "to":"auction1", "quantity":"{0}.0000 EOS".format(price), "memo":"m"}
        r = eosapi.push_action('eosio.token', 'transfer', msg, {'hello':'active', 'eosio.token':'active'})
        assert r
...
def withdraw():
    ...
    with producer:
        sender = eosapi.N('hello')
        msg = int.to_bytes(sender, 8, 'little')
        r = eosapi.push_action('auction1', 'withdraw', msg, {'auction1':'active', 'hello':'active'})
        assert r
    '''
#multi_index.hpp 695: cannot modify objects in table of another contract
    with producer:
        ...
        r = eosapi.push_action('auction1', 'withdraw', msg, {'auction1':'active'})
