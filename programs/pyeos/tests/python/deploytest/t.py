...
def init(func):
    def func_wrapper(*args, **kwargs):
        prepare('deploytest', 'deploytest.py', 'deploytest.abi', __file__)
        func(*args, **kwargs)
    return func_wrapper

@init
def test(name='mike'):
    r = eosapi.push_action('deploytest','sayhello', name, {'deploytest':'active'})
...
@init
def play():
    r = eosapi.push_action('deploytest', 'play', '', {'deploytest':'active'})
    assert r
...
def test2(count=100):
    ...
    for i in range(count):
        action = ['deploytest', 'sayhello', str(i), {'deploytest':'active'}]
...
def test3(count=100):
    actions = []
    for i in range(count):
        act = [N('deploytest'), N('sayhello'), [[N('deploytest'), N('active')]], b'hello,world%d'%(i,)]
        actions.append([act])
...
def deploy():
    ...
    with open(src_file, 'rb') as f:
        src = f.read()
    r = eosapi.push_action('deploytest','deploy', src, {'deploytest':'active'})
    assert r

@init
def deploytest():
    r = eosapi.push_action('deploytest','deploytest', '', {'deploytest':'active'})
    assert r
