...
variant number_from_stream(T& in) {
   ...
   if (str == "-." ||
       str ==
           ".")  // check the obviously wrong things we could have encountered
      FC_THROW_EXCEPTION(
          parse_error_exception,
...
variant json::from_string(const std::string& utf8_str, parse_type ptype) {
   try {
      ...
      switch (ptype) {
         ...
         case strict_parser:
            return json_relaxed::variant_from_stream<std::stringstream, true>(
                in);
         case relaxed_parser:
            return json_relaxed::variant_from_stream<std::stringstream, false>(
                in);
         default:
            FC_ASSERT(false, "Unknown JSON parser type {ptype}",
                      ("ptype", ptype));
      }
   }
   ...
   try {
      ...
      try {
         while (true) {
            ...
            result.push_back(
                json_relaxed::variant_from_stream<std::stringstream, false>(
                    in));
         }
   ...
   for (uint32_t i = 0; i < v.size(); ++i) {
      switch (v[i]) {
         ...
         case ',':
            ...
            break;
         case 'n':
            // If we're in quotes and see a \n, just print it literally but
            // unset the escape flag.
            if (quote && escape) escape = false;
...
variant json::from_file(const fc::path& p, parse_type ptype) {
   ...
   // std::ios::binary );  buffered_istream bi( tmp );
   boost::filesystem::ifstream bi(p, std::ios::binary);
   switch (ptype) {
      case legacy_parser:
         return variant_from_stream<boost::filesystem::ifstream, legacy_parser>(
             bi);
      case legacy_parser_with_string_doubles:
...
/*
 ...
 legacy_parser_with_string_doubles>( in ); case strict_parser: return
 json_relaxed::variant_from_stream<buffered_istream, true>( in ); case
 relaxed_parser: return json_relaxed::variant_from_stream<buffered_istream,
 false>( in ); default: FC_ASSERT( false, "Unknown JSON parser type {ptype}",
 ("ptype", ptype) );
 }
 }
 */
...
bool json::is_valid(const std::string& utf8_str, parse_type ptype) {
   ...
   switch (ptype) {
      case legacy_parser:
         ...
         break;
      case legacy_parser_with_string_doubles:
         variant_from_stream<std::stringstream,
                             legacy_parser_with_string_doubles>(in);
         break;
      case strict_parser:
