...
public:
   ...
   virtual void set_dir(const boost::filesystem::path& p) { dir = p; }
   ...
   virtual void set_timeout(const std::chrono::seconds& t);

   /// @see wallet_manager::set_timeout(const std::chrono::seconds& t)
   /// @param secs The timeout in seconds.
   virtual void set_timeout(int64_t secs) { set_timeout(std::chrono::seconds(secs)); }
   ...
   virtual chain::signed_transaction sign_transaction(const chain::signed_transaction& txn, const flat_set<public_key_type>& keys,
   ...
   /// @throws fc::exception if corresponding private keys not found in unlocked wallets
   virtual chain::signature_type sign_digest(const chain::digest_type& digest, const public_key_type& key);
   ...
   virtual void save_wallet(const std::string& name);
   ...
   virtual void lock(const std::string& name);
   ...
   virtual void unlock(const std::string& name, const std::string& password);
   ...
   virtual void import_key(const std::string& name, const std::string& wif_key, bool save=true);
   ...
   virtual void remove_key(const std::string& name, const std::string& password, const std::string& key);
   ...
   virtual string create_key(const std::string& name, const std::string& key_type);
