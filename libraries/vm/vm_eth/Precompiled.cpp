...
ETH_REGISTER_PRECOMPILED(sha256)(bytesConstRef _in)
{
   ...
   auto data = _in.toBytes();
   get_vm_api()->sha256((char *)data.data(), data.size(), &hash);
   return {true, dev::bytes(hash.hash, hash.hash+sizeof(hash.hash))};
}
...
ETH_REGISTER_PRECOMPILED(ripemd160)(bytesConstRef _in)
{
   ...
   auto data = _in.toBytes();
   get_vm_api()->ripemd160( (char *)data.data(), data.size(), &hash );
