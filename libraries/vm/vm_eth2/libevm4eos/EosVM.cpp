...
uint64_t EosVM::memNeed(u256 _offset, u256 _size)
{
...
uint64_t EosVM::decodeJumpDest(const byte* const _code, uint64_t& _pc)
{
...
uint64_t EosVM::decodeJumpvDest(const byte* const _code, uint64_t& _pc, byte _voff)
{
...
//
void EosVM::adjustStack(unsigned _removed, unsigned _added)
{
	m_SP = m_SPP;
...
owning_bytes_ref EosVM::exec(u256& _io_gas, ExtVMFace& _ext, OnOpFunc const& _onOp)
{
	m_io_gas_p = &_io_gas;
	...
	m_onOp = _onOp;
	m_onFail = &EosVM::onOperation; // this results in operations that fail being logged twice in the trace
	m_PC = 0;

	try
	{
		// trampoline to minimize depth of call stack when calling out
		m_bounce = &EosVM::initEntry;
		do
			(this->*m_bounce)();
		while (m_bounce);
